...
public class CategoryHateoasController {
    private final CategoryService categoryService;

    @Autowired
    public CategoryHateoasController(CategoryService categoryService) {
    ...
    public ResponseEntity<EntityModel<Category>> createCategory(@RequestBody Category category) {
        ...
        categoryResource.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CategoryHateoasController.class).getCategoryById(createdCategory.getId())).withSelfRel());
    ...
    public ResponseEntity<EntityModel<Category>> getCategoryById(@PathVariable Integer id) {
        ...
        if (category != null) {
            ...
            categoryResource.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CategoryHateoasController.class).getCategoryById(id)).withSelfRel());
    ...
    public ResponseEntity<List<EntityModel<Category>>> getAllCategories() {
        ...
        List<EntityModel<Category>> categoryResources = categories.stream()
                .map(category -> {
                    EntityModel<Category> categoryResource = EntityModel.of(category);
                    categoryResource.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CategoryHateoasController.class).getCategoryById(category.getId())).withSelfRel());
    ...
    public ResponseEntity<EntityModel<Category>> updateCategory(@PathVariable Integer id, @RequestBody Category category) {
        ...
        if (existingCategory != null) {
            ...
            categoryResource.add(WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(CategoryHateoasController.class).getCategoryById(updatedCategory.getId())).withSelfRel());
            return ResponseEntity.ok(categoryResource);
        } else {
            return ResponseEntity.notFound().build();
        }
